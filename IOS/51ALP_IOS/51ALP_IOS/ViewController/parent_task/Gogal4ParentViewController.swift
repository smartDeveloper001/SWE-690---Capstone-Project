//
//  Target4ParentViewController.swift
//  51ALP_IOS
//
...
class Gogal4ParentViewController: UITableViewController {
    ...
    convenience init(taskSummaryInfo:TaskSummaryInfo,style: UITableView.Style){
        ...
        self.taskSummaryInfo = taskSummaryInfo
        self.cellsData = taskSummaryInfo.courseToGogal[taskSummaryInfo.selectedCourseId]!
        self.sectionName = taskSummaryInfo.selectedCourseName
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(Target4ParentTableViewCell.self, forCellReuseIdentifier: cellIdentifier)
    ...
    func prepareNavigationItem() {
        ...
        navigationItem.titleLabel.text = "任务所在目标"
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath as IndexPath) as! Target4ParentTableViewCell
        
        print(indexPath)
        var image = UIImage(named: "kids_course.png")
        image = image?.scaleImage(scaleSize:0.5)
        cell.myImageView.image = image
        cell.name.text = cellsData[indexPath.row].goalName!
        
        let badgeValue = self.taskSummaryInfo.gogalToBadge[cellsData[indexPath.row].id]!
        if badgeValue > 0 {
        ...
        cell.gogalId = cellsData[indexPath.row].id
        return cell
    }
    ...
    func handleDetail(cell: Target4ParentTableViewCell) {
    ...
    func handleNext(cell: Target4ParentTableViewCell) {
        
     
        
        self.taskSummaryInfo.selectedGogalId =  cell.gogalId
        self.taskSummaryInfo.selectedGogalName = cell.name.text!
        let stc = Break4ParentViewController(taskSummaryInfo:self.taskSummaryInfo,style:.plain)
...
class Target4ParentTableViewCell: UITableViewCell {
    var myTableViewController:Gogal4ParentViewController?
    var gogalId = 0
